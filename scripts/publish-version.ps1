...
Write-Host "[INFO] Creating version $tag" -ForegroundColor Cyan
...
if ($gitStatus) {
    if ($DryRun) {
        Write-Warning "Dry run: uncommitted changes detected. Continuing because no writes will be made."
        git status --short
    } else {
        Write-Error "There are uncommitted changes. Please commit or discard them before creating a version."
        git status --short
        exit 1
    }
}
...
Write-Host "[INFO] Synchronizing with the remote repository..." -ForegroundColor Cyan
git fetch origin
...
if ($localCommit -ne $remoteCommit) {
    Write-Warning "Your local branch is not synchronized with origin/$currentBranch"
...
if ($existingTag) {
    if ($DryRun) {
        Write-Warning "Dry run: the tag '$tag' already exists. Command preview only."
    } else {
...
Write-Host "`n[INFO] Latest commits:" -ForegroundColor Cyan
git log --oneline -5

Write-Host "`n[INFO] The tag will be created: $tag" -ForegroundColor Green
Write-Host "[INFO] On commit: $(git rev-parse --short HEAD)" -ForegroundColor Gray
...
# Create the tag
Write-Host "`n[INFO] Creating tag $tag..." -ForegroundColor Cyan
...
# Publish the tag
Write-Host "[INFO] Publishing tag to the remote repository..." -ForegroundColor Cyan
git push origin $tag
...
Write-Host "`n[SUCCESS] Version $tag created and published successfully!" -ForegroundColor Green
Write-Host "[INFO] The GitHub Actions workflow will start automatically" -ForegroundColor Cyan
Write-Host "[INFO] Monitor the progress at: https://github.com/evaristocuesta/Clovance/actions" -ForegroundColor Blue

Write-Host "`n[INFO] The images will be published at:" -ForegroundColor Cyan
...
if ($isPreRelease) {
    Write-Host "`n[INFO] This is a pre-release. Tags that will be created:" -ForegroundColor Yellow
    Write-Host "  - $Version (full version)" -ForegroundColor Gray
    Write-Host "  - $currentBranch-<sha> (commit SHA)" -ForegroundColor Gray
    Write-Host "`n[NOTE] Pre-releases do not update production tags (major, minor, latest)" -ForegroundColor DarkYellow
} else {
    Write-Host "`n[INFO] The following tags will also be created:" -ForegroundColor Yellow
    $parts = $Version.Split('.')
    Write-Host "  - $Version (full version)" -ForegroundColor Gray
